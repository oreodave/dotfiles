...
Set the warning level to just errors, so we don't get crap on for
...
buffer).  I try to make sure we're reverting wherever possible.
#+begin_src emacs-lisp
...
Please note ~+cfg/clean-buffers-keep~, which defines a list of buffers
that must be preserved for any operation suggested by the above.
#+begin_src emacs-lisp
...
"default" face to some fixed value.  The default face is usually the
face that all other faces in a theme inherit from.  Setting it here
naturally adjusts all font sizes for any face and thus for the whole
theme.

I add this function to ~enable-theme-functions~ such that loading a
theme will forcefully adjust the font size, as well as
...
But, sometimes you really do want to globally adjust the font.  To
make this as easy as possible, I've introduced a scaling factor that
can be adjusted through the ~+cfg/reset|incf|decf-font-scale~
functions.  This motivates the construction of a [[*Hydra][Hydra]],
which see.
...
(defun +cfg/--compute-font-size ()
  (ceiling
   (* +cfg/default-font-size +cfg/scale)))
...
(defun +cfg/reset-font-scale ()
  (interactive)
  (setf +cfg/scale 1)
  (message "+cfg/scale = %s" +cfg/scale)
  (mapc #'enable-theme custom-enabled-themes))

(defun +cfg/incf-font-scale ()
  (interactive)
  (incf +cfg/scale 0.1)
  (message "+cfg/scale = %s" +cfg/scale)
  (mapc #'enable-theme custom-enabled-themes))

(defun +cfg/decf-font-scale ()
  (interactive)
  (decf +cfg/scale 0.1)
  (message "+cfg/scale = %s" +cfg/scale)
  (mapc #'enable-theme custom-enabled-themes))
...
themes that ship with Emacs.  So why not use them?
...
provide a minimal fringe style with only 5 pixels on the left
provided.
...
(use-package zoom
  :ensure t
  :defer t
  :hook (elpaca-after-init-hook . zoom-mode)
  :init
...
(use-package magit
  ...
  (defun magit-push-all-remotes ()
    ...
    (let* ((current-branch (magit-get-current-branch))
      ...
      (mapc (lambda (remote)
              (magit-run-git-async "push" "-v" remote current-branch))
            (magit-list-remotes))))

  (transient-append-suffix 'magit-push "e"
    '("P" "Push to all remotes" magit-push-all-remotes)))
#+end_src
*** Magit Forge
...
(use-package eldoc-box
  ...
  :init
  (setopt eldoc-box-max-pixel-height 350))
#+end_src
** Flycheck
...
(use-package tab-bar
  ...
  (defun +tab-bar/full-name ()
    (let* ((window (or (minibuffer-selected-window)
      ...
      (if project
          (format "%s[%s]" (project-name project) buffer-name)
        buffer-name)))
...
symbols around as I wanted.  It's really fun to use.
#+begin_src emacs-lisp
(use-package jagger
...
(use-package undo-tree
  ...
  (setq undo-tree-auto-save-history t
        ...
        undo-limit 400000
        undo-tree-limit 400000
        undo-strong-limit 800000
        undo-tree-strong-limit 800000
        undo-outer-limit 1600000
        undo-tree-outer-limit 1600000)
  (with-eval-after-load "evil"
...
#+end_src
** Boxes
See ~man boxes~; great command for creating little cute boxes.
This command (bound to ~SPC i b~) allows one to invoke ~boxes~.  By
default it will ask for the input string and that is all.  With ~C-u~,
one can specify further options for the raw command line utility.
