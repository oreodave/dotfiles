...
The prose is completely ignored in the final product, so I can write
to my hearts content.  Pretty cool, right?  This style of coding is
...
- Sections tagged with =BACKLOG= are not compiled into the final
...
the current buffer are all killed.  With ~C-u~, all buffers are
killed.

~+cfg/clean-buffers-keep-cond~ is a variable that filters out certain
buffers from the operations of ~+cfg/clean-buffers~.  I use this to
preserve stuff like internal hidden buffers and the =*scratch*=
buffer.
#+begin_src emacs-lisp
...
Completion preview mode comes out of the box with Emacs 30.  Its
provides a very simple view
...
like [[info:emacs#Compilation Mode][Compilation Mode]] or
[[*Eglot][Eglot]] do require fringes to provide arrows on the left
side of the window.  Hence I provide a minimal fringe style with only
5 pixels on the left provided.
...
lines) and scroll-conservatively to the same value so it'll keep the
cursor centred.
...
(use-package vundo
  ...
  (setq undo-limit        (* 3 1024 1024)   ; 16MB
        undo-strong-limit (* 6 1024 1024)   ; 24MB
        undo-outer-limit  (* 12 1024 1024)) ; 128MB
  (with-eval-after-load "evil"
...
(use-package emacs
  ...
  (whitespace-mode-hook . display-fill-column-indicator-mode)
  :init
...
#+end_src
*** Truncate lines
2026-08-01: To deal with the regression, we'll have to begrudgingly
accept truncate-lines as an alternative.
#+begin_src emacs-lisp
(use-package emacs
  :demand t
  :config
...
AST, but they also work for C-style languages as well.  Here I
...
(use-package compile
  ...
  (setq compilation-scroll-output 'first-error
        ...
        compilation-environment '("CLICOLOR_FORCE=1"))
...
(use-package compile
  ...
  (thread-last
    `(,(rx (and
            ...
            (group (+ digit)) ":"))
      1 2 3)
    (setf (cdr (assoc 'rustc-panics compilation-error-regexp-alist-alist))))
  (thread-last
    `(,(rx (and
            line-start
            "  "
            ...
            (group (+ digit)) ":"))
      0 1 2)
    (setf (cdr (assoc 'rustc compilation-error-regexp-alist-alist)))))
#+end_src
** BACKLOG Racket
:PROPERTIES:
:header-args:emacs-lisp: :tangle no :results none
...
(use-package gptel
  ...
  (defun +gptel/switch-buffer ()
    ...
    (thread-last
      ...
      (lambda (x))
      (read-buffer "Choose gptel buffer: " nil t)
      (switch-to-buffer))))
#+end_src
**** LLM tools
